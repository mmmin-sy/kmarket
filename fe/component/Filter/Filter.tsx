		...
		<div className="static px-6 py-6">
			<div className="relative h-[40px]">
						...
						<div className="w-full absolute top-0 left-0 h-[4px]">
							<div className="relative">
								{min <= max ? (
									...
									<div
										className="absolute w-[16px] h-[16px] top-[-6px] rounded-lg bg-custom-gray-lighter"
										style={{left: '-8px' }}
									>
									</div>
								)}
				...
				<div className="absolute left-0 top-4">
					{min <= max && `${lowerValue} ${unit}`}
					{min > max && <ErrorMessage message="min is greater than max!"/>}
				</div>
				<div className="absolute right-0 top-4">
					{min <= max && `${upperValue} ${unit}` }
					{ min > max && <ErrorMessage message="max is smaller than min!" /> }
				</div>
			</div>
		</div>
	)
